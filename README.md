...
- **逻辑帧与显示帧解耦**：默认 `frameskip=1`，核心仍执行约 59.7 帧/秒，只提交约 30 帧/秒；
  ...
  逻辑速度。画面以原生 `240x160 RGB565` 路径输出，不做缩放、色彩校正或帧混合。
- **直接 framebuffer 输出**：通过 SDK 正式接口动态获取 `240x320 RGB565` framebuffer，
  在内存中合成顶部游戏画面与底部控制层后直接提交，并按固件报告的方向处理 180 度旋转；
  不依赖固定 framebuffer 地址。仅在获取或提交失败时退回 `bda_gui_render_picture`。触摸控制层
  只在按键、声音或帧率状态变化时重绘，重绘时直接覆盖背景，不再先对 76.8 KiB 像素区做
  冗余清零。
...
R45 直接 framebuffer 构建在 9588 模拟器中使用 16 MiB Emerald ROM 连续运行 6191 个
逻辑帧。去除启动段后的稳定区间平均为 `59.77` 逻辑 FPS、`29.88` 显示 FPS，范围分别为
`59.70-59.85` 和 `29.85-29.92`；音频生成率为 `22040-22095 Hz`。累计 3098 次 framebuffer
提交均成功，视频提交、音频短写、丢弃和背压超时均为 0，正常退出结果为 `RESULT=PASS`。
该数据用于模拟器回归比较，真机表现仍会受 ROM、固件版本和触摸事件负载影响。
