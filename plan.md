...
- [x] gpSP 每累计 8192 cycles 子帧采样一次输入，并在同一个 `update_gba()` 中更新 KEYINPUT
  和处理按键 IRQ。
...
1. 用 R32 真机连续运行 10 分钟和 30 分钟，逐个测试方向、A/B/L/R、Start/Select、声音、帮助
   和换 ROM；日志应显示 `INPUT_ARCH=RAW_EVENT_SELF_MANAGED`、`WINDOW_EVENT_PUMP=DISABLED`，
   且 `raw_poll` 约为核心帧数的 34 倍、`raw_max<=8`、音频 short/drop/backpressure 为 0。
2. 真机比较短按、拖动和长按触摸的响应时间，确认首个 DOWN 后 MOVE 连续到达、抬起不粘键，
   并验证实体 Escape 长按可以完成保存和正常退出。
3. 分别补 SRAM 与 EEPROM fixture，并对 Emerald 的 Flash 1M 最新槽做损坏回退测试，
   将真实游戏路径纳入可重复的存档回归。
4. 引入公开 homebrew 与 mGBA test suite，建立 CPU/PPU/DMA/timer/save 的解释器与
   dynarec 回归矩阵，不把商业 ROM 作为唯一正确性依据。
5. 实现 frameskip、音量、按键方案和 ROM cache 的最小配置；补精确 fps/underrun 指标，
   根据真机数据收敛 pacing 和控制层 dirty rect。
6. 完成 30 分钟资源稳定性、十次启动/退出、许可证组合审查和可复现发布构建。
