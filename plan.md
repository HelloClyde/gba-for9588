...
- [x] gpSP 每累计 16384 cycles 子帧采样一次输入，并在同一个 `update_gba()` 中更新 KEYINPUT
  和处理按键 IRQ。
...
1. 用 R34 真机连续运行 10 分钟和 30 分钟，逐个测试方向、A/B/L/R、Start/Select、声音、帮助
   和换 ROM；日志应显示 `INPUT_ARCH=RAW_EVENT_SELF_MANAGED`、`WINDOW_EVENT_PUMP=DISABLED`，
   且 `raw_poll` 约为核心帧数的 17 倍、`raw_max<=8`、音频 short/drop/backpressure 为 0。
